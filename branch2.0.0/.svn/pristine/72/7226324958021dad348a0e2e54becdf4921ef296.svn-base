...
Page({
  ...
  formIdTo(e) {
    var that = this;
    var pageurl = e.detail.target.dataset.url;
    // console.log(url)
    var form_id = e.detail.formId;
    var url = 'api/log/emptynull';
    var data = {
  ...
  jump_live: util.throttle(function (e) {
    ...
    // var data = {};
    // data.id = id;
    // data.live_data_id = live_data_id;
  ...
  feiyu_btnSpecialDetail: util.throttle(function (e) {
    var url = e.currentTarget.dataset.url;
    var app = getApp();
    app.aldstat.sendEvent('首页-精选专题商品');
    wx.navigateTo({
      url: '../../' + url,
    })
  }, 1000),
  ...
  feiyu_onClickPath: util.throttle(function (e) {
    ...
    var index = e.currentTarget.dataset.index;
    var app = getApp();
    app.aldstat.sendEvent('首页banner', {
      'index': index + '',
    });
    wx.navigateTo({
      url: '../../' + app_url,
  ...
  alert_btn(e) {
    ...
    var live_data_id = e.currentTarget.dataset.live_data_id;
    console.log(live_data_id);
    var that = this;
    var app = getApp();
    app.aldstat.sendEvent('首页-开播提醒', {
      'id': live_id + ''
    });
  ...
  onLoadData() {
    ...
    if (my_home_flag == 1) { //妃鱼首页
      ...
      var my_home_id = wx.getStorageSync('my_home_id');
      data.user_id = wx_id;
      data.shop_id = my_home_id;
      url = "api/wxapp/shome";
    } else {
      url = "api/wxapp/mhome";
    }
      ...
      if (res.data.returnCode == 0) {
        if (my_home_flag == 1) {
          ...
          that.setData({
            is_auth: is_auth,
            isShow: 0,
            is_mobile:res.data.data.g_coupon.is_mobile,
            isHasStorage: 1,
            info: res.data.data,
  ...
  takeRedPack() {
    var that = this;
    if (that.data.is_mobile == 0) {
      getApp().takeGcoupon = 1;
      that.setData({
        showRedPack: 0,
        noscroll: 0,
        tipsClick: 0
  ...
  saveImgToPhotosAlbumTap: function () {
    var that = this;
    console.log(that.data.share_image_url)
    wx.downloadFile({
