...
Page({
  ...
  jumpProduct: util.throttle(function (e) {
    ...
    var app = getApp();
    app.aldstat.sendEvent('购物车列表页-商品', {
  ...
  btnEdit: function (e) {
    ...
    var app = getApp();
    app.aldstat.sendEvent('购物车列表页-勾选店铺');
    ...
    //       console.log(that.data.shopList[i].product_skus[j]);
    //       that.setData({
    //         isShowSku: that.data.shopList[i].product_skus[j],
    //         inputValue: that.data.shopList[i].product_skus[j].cart_num,
    //       })
    //     }
  ...
  btnPayOrder: function (e) {
    ...
    var app = getApp();
    app.aldstat.sendEvent('购物车列表页-结算按钮');
    wx.navigateTo({
  ...
  btnCheck: function (e) {
    ...
    var id = e.currentTarget.dataset.id;
    var app = getApp();
    app.aldstat.sendEvent('购物车列表页-勾选商品', {
      'id': id + ''
    });
    for (var i = 0; i < that.data.shopList.length; i++) {
      for (var j = 0; j < that.data.shopList[i].product_skus.length; j++) {
  ...
  goToShop: function (e) {
    ...
    var app = getApp();
    app.aldstat.sendEvent('购物车列表页-店铺昵称', {
      'shop_id': id,
    });
    wx.navigateTo({
      url: '/' + shop_app_url,
